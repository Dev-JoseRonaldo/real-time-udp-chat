...
def send_packet(message, sender, destination_address, origin_adress=None, nickname=None, seq_num=None, ack_num=None):
    ...
    if message == "FYN-ACK" or message == "ACK" or message == "" or message == "SYN" or message == "SYN-ACK": # Se pacote enviado for de reconhecimento
        ...
        while contents: 
            ...
            ack_received = ack_received_control.get_ack_received(origin_adress, nickname)

            # Aguarda até que ack seja recebido ou timeout estoure
            ...
            if fragment_sent:
                ack_received_control.update_ack_received(origin_adress, nickname, False) # Reseta status do ack
